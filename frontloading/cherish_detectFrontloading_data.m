...
%run statistics on the data for each rat 
for XX = 1:numel(subjects);
    data = Dataset(:,XX);
    

    if isnan(data(1)) %check if the first location has a NaN. This means there was no data so the model won't work 
        subjectClassification(XX,1:2) = {subjects(XX), "NaN"};
        changePoint_all = [changePoint_all; NaN];
        firstSlope_all = [firstSlope_all; NaN];
        secondSlope_all = [secondSlope_all; NaN];
        continue; 
    elseif data(1) ~= 0
    ...
    % if chPt(1) > chPt(2) && chPt(1) > chPt(3)
    %     %if criteria 1 is not met, classify subject as inconclusive.  
    %     Criteria1 = 0;
    %     subjectClassification(XX,1:2) = {subjects(XX), "Inconclusive1"};
    ...
    % elseif chPt(1) < chPt(2) && chPt(1) > chPt(3)
    %     Criteria1 = 0;
    %     subjectClassification(XX,1:2) = {subjects(XX), "Inconclusive3"};
    %     changePoint_all = [changePoint_all; chPt(1)];
    %     firstSlope_all = [firstSlope_all; stats_pre.beta(2)];
    %     secondSlope_all = [secondSlope_all; stats_post.beta(2)]; 
    % elseif  chPt(1) > chPt(2) && chPt(1) < chPt(3)
    %     Criteria1 = 0;
    %     subjectClassification(XX,1:2) = {subjects(XX), "Inconclusive2"};
    %     changePoint_all = [changePoint_all; chPt(1)];
    %     firstSlope_all = [firstSlope_all; stats_pre.beta(2)];
    %     secondSlope_all = [secondSlope_all; stats_post.beta(2)]; 
    
    %if the first change point (change point with the best fit from the parcs model) is within 
    % the last half of the session, set criteria1 to 0/false
    if chPt(1) > half_session
        Criteria1 = 0;
