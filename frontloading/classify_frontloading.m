...
% can be used on drinking and DD data

%% Calculate data for each rat

% calculate cumulative RAP data
time = 3600;
consumptionOverTime = calc_cumulativeConsumption(RAP_all, RAP_lickTmSerMtx, RAP_totalLicks, cumulative_type = "lick", time = time);
...
Metabolic_Rate = 0.0001; %enter the metabolic rate in g/kg/your timescale (min or sec). Currently is in 
% g/kg/sec. Can change based Cherish detectFrontloading_data code 
...
for i = 1:numel(days)
    ...
    for j = 1:size(data{1},1)
        try 
            ...
            %data
            % CHANGE Y-INTERCEPT TO 3 IF USING DD DATA and 0 IF USING
            % DRINKING DATA
...
for i = 1:numel(all_results)
    slope2_gkg = [slope2_gkg all_results{i}(:, 2)];
end 
