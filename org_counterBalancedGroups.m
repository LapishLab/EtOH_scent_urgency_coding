...
Control_group = ratsInfo.sex == "M" & ratsInfo.treatment == "Control";
EtOH_group1 = ratsInfo.sex == "M" & ratsInfo.treatment == "EtOH" & ratsInfo.DD_scent_counterbalanced_identifier == 1;
EtOH_group2 = ratsInfo.sex == "M" & ratsInfo.treatment == "EtOH" & ratsInfo.DD_scent_counterbalanced_identifier == 2;
...
plotLPError(trials, data{2}(logical(EtOH_group1 + EtOH_group2), :), 'mn', '+-', 'Color', 'black')
legend(["EtOH" "Water" "Day Before"])
ylim([0 6])
title("First Day of Scent DD: Males")
hold off; 
...
for day = 1: size(days, 2)
    for group = 1:size(noise_groups, 1)
        noise_ratIndex(:,day) = sum([noise_ratIndex(:,day) (ratsInfo.USV_playback_group == noise_groups(group, day) & ratsInfo.sex == "F" & ratsInfo.treatment == "EtOH")], 2);
        happy_ratIndex(:,day) = sum([happy_ratIndex(:,day) (ratsInfo.USV_playback_group == happy_groups(group, day) & ratsInfo.sex == "F" & ratsInfo.treatment == "EtOH")], 2);
        sad_ratIndex(:,day) = sum([sad_ratIndex(:,day) (ratsInfo.USV_playback_group == sad_groups(group, day) & ratsInfo.sex == "F" & ratsInfo.treatment == "EtOH")], 2);
    end
...
%plot the effects of the first day 
dayEffects = figure()
plotLPError(trials, data{days(1)}(logical(noise_ratIndex(:,1)), :), 'mn', 'o-', 'Color', 'black')
hold on;
plotLPError(trials, data{days(1)}(logical(happy_ratIndex(:,1)), :), 'mn', 'x-', 'Color', 'red')
plotLPError(trials, data{days(1)}(logical(sad_ratIndex(:,1)), :), 'mn', '+-', 'Color', 'blue')
title("First Day USV Playback: Female EtOH Consumers")
legend(["Noise" "Happy" "Sad"])
ylim([0 6])
hold off;
