...
date_range = datetime(2025, 11, 3):datetime(2025, 11, 14);
date_range.Format = "yyyy-MM-dd";
date_range = string(date_range);
match = contains(t.data_path, date_range) & contains(t.data_path, "Prat_Urgency");
specific_table = t(match, :); 

%find import excel sheet with issue times 
dirPath = "E:\072125_121225_Prat_urgency\DD";
excelFile = "090225_DD_data.xlsx";
...
%and exposure type
dataInfo_CSV = table('Size', [0 5], ...
          'VariableTypes', {'string', 'string', 'string', 'string', 'string'}, ...
          'VariableNames', {'audio_file_path', 'subject', 'sex', 'treatment', 'issueTime'});
...
for row = 1:size(specific_table,1)
    try 
        ...
        dataInfo_CSV(end+1, :) = {specific_table.data_path(row), specific_table.subject(row), sex, treatment, issueTime};
    catch 
  ...
  % Save the updated table back to a new CSV file
    writetable(dataInfo_CSV, fullfile(dirPath, 'DD_audioFiles.csv'));